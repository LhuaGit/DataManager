...
import com.manager.dao.UserRoleMapper;
import com.manager.entity.UserDTO;
import com.manager.entity.UserRoleDTO;
import com.manager.entity.UserRolesVO;
import com.manager.entity.UserSearchDTO;
import com.manager.model.Role;
import com.manager.model.User;
import com.manager.model.UserRoleKey;
...
public class UserServiceImpl implements UserService {
    ...
    public PageDataResult getUsers(UserSearchDTO userSearch, int page, int limit) {
        ...
        if (null != userSearch) {
            if (StringUtils.isNotEmpty(userSearch.getInsertTimeStart())
                ...
                userSearch.setInsertTimeEnd(DateFormatUtils.format(new Date(), "yyyy-MM-dd HH:mm:ss"));
            } else if (StringUtils.isEmpty(userSearch.getInsertTimeStart())
                    && StringUtils.isNotEmpty(userSearch.getInsertTimeEnd())) {
                userSearch.setInsertTimeStart(DateFormatUtils.format(new Date(), "yyyy-MM-dd HH:mm:ss"));
            }
            if (StringUtils.isNotEmpty(userSearch.getInsertTimeStart())
                    && StringUtils.isNotEmpty(userSearch.getInsertTimeEnd())) {
    ...
    public String sendMsg(UserDTO user) {
        ...
        if (null != existUser && existUser.getMobile().equals(user.getMobile())) {
            ...
            // 发送短信验证码 ok、no
            return mobileCode + "(验证码)，如不是本人操作，请忽略此消息。"
                    + user.getMobile();
        } else {
    ...
    public String sendMessage(int userId, String mobile) {
        ...
        this.userMapper.updateByPrimaryKeySelective(user);
        // 发送短信验证码 ok、no
        return mobile_code + "(验证码)，如不是本人操作，请忽略此消息。"
                + user.getMobile();
    }
